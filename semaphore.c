...
//is the producer. It enqueues the value that was saved from the previous problem into the buffer.
        ...
        *counter = 0;
        ...
        while (stored_counter != 0)
        {
            ...
            *(buffer + buffer_idx) = msg_number;
            ...
            stored_counter--; //represents the remaining number of messages to be processed and written to the buffer.
            if (DISPLAY_MONITOR)
            {
...
int main()
{
    ...
    for (int i = 0; i < NUM_COUNTERS; i++)
    {
        ...
        pthread_create(&mCounterThreads[i], NULL, mCounter, (void *)initCounterArg(i + 1, mut_counter)); //receives a unique identifier and a pointer to the mutex counter.
    }
